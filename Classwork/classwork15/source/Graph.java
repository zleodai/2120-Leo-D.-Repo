...
public class Graph {
   public int[][] adjaMatrix; // The adjacency matrix represents the graph
   public boolean[] visited; // Element is true if vertex is visited
   ...
   public int convertToChar(int index) {
      return ('A' + index);
   }

   // Check if two vertexs are connected
   public boolean areTwoVertexsConnected(char vertexOne, char vertexTwo) {
      return (adjaMatrix[convertToIndex(vertexOne)][convertToIndex(vertexTwo)] == 1);
   }
}