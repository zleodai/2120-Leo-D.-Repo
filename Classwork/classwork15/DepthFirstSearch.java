...
class DepthFirstSearcher {
    ...
    public static void dfs(Graph theGraph, char currentVertex) {
        ...
        for (char nextVertex = 'A'; nextVertex <= ('A' + TEST_GRAPH_SIZE - 1); nextVertex++) {
            if (DEBUG_ON)
                System.out.print("  Checking " + nextVertex + " >> ");

            if (theGraph.areTwoVertexsConnected(currentVertex, nextVertex)) {

                if (DEBUG_ON)
                    System.out.println("  Vertexs " + currentVertex + " and " + nextVertex + " ARE connected.");

                if (!theGraph.wasVisited(nextVertex)) {

                    if (DEBUG_ON)
                        System.out.println("Searching with " + nextVertex + " as root of tree.");
                    ...
                    if (DEBUG_ON)
                        System.out.println("    But Vertex " + nextVertex + " was already visited.");

                }
            } else {

                if (DEBUG_ON)
                    System.out.println("  Vertexs " + currentVertex + " and " + nextVertex + " are NOT connected.");
